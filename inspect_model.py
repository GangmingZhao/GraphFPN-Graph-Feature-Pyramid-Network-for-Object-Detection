...
refined_anchors = transforms.delta2bbox(positive_anchors, positive_target_deltas, (0., 0., 0., 0.), (0.1, 0.1, 0.2, 0.2))
...
proposals = model.rpn_head.get_proposals(rpn_probs, rpn_deltas, batch_metas)

rois = proposals
pooled_regions = model.roi_align((rois, rcnn_feature_maps, batch_metas), training=training)
rcnn_class_logits, rcnn_probs, rcnn_deltas = model.bbox_head(pooled_regions, training=training)

detections_list = model.bbox_head.get_bboxes(rcnn_probs, rcnn_deltas, rois, batch_metas)
...
visualize.display_instances(ori_img, res['rois'], res['class_ids'], val_dataset.get_categories(), scores=res['scores'])